...
inline void die_with_message(const char* msg, const char* file, size_t line) {
    std::ostringstream oss;
    oss << msg << " @ " << file << ':' << line;
    die_with_message(oss.str());
}

inline void die_with_message(const std::string& msg, const char* file, size_t line) {
...
class BTree {
   ...
   public:
    ...
    void bulk_load(Iterator ibegin, Iterator iend) {
        ...
        for (int level = 2; num_parents != 1; ++level) {
            ...
            TLX_BTREE_PRINT("BTree::bulk_load, level " << level << ": " << num_children << " children in "
   ...
   public:
    ...
    void erase(iterator iter) {
        TLX_BTREE_PRINT("BTree::erase_iter(" << iter.curr_leaf << "," << iter.curr_slot << ") on btree size "
                                             << size());
   ...
   private:
    ...
    result_t erase_one_descend(const key_type& key, node* curr, node* left, node* right, InnerNode* left_parent,
        ...
        } else  // !curr->is_leafnode()
        {
            ...
            result_t result = erase_one_descend(key, inner->childid[slot], myleft, myright, myleft_parent,
    ...
    result_t erase_iter_descend(const iterator& iter, node* curr, node* left, node* right, InnerNode* left_parent,
        ...
        } else  // !curr->is_leafnode()
        {
            ...
            while (slot <= inner->slotuse) {
                ...
                result = erase_iter_descend(iter, inner->childid[slot], myleft, myright, myleft_parent, myright_parent,
                                            inner, slot);
