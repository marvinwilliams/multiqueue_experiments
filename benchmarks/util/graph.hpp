...
static Graph from_file(std::filesystem::path const& graph_file) {
    ...
    std::exclusive_scan(graph.nodes.begin() + 1, graph.nodes.end(), graph.nodes.begin() + 1, 0);
    for (auto& edge : edge_list) {
        graph.edges[graph.nodes[edge.first]++] = edge.second;
    }
    return graph;
}
