    ...
    ctx.execute_synchronized_blockwise_timed(
        ...
        [id = ctx.get_id()](key_type* begin, key_type* end) {
            std::generate(begin, end, [id]() {
                return std::uniform_int_distribution<key_type>(
                    settings.min_key, settings.max_key)(thread_data[id].rng);
            });
        });
    ctx.execute_synchronized_blockwise_timed(
        "generate_keys", keys,
...
void delete_all(thread_coordination::Context& ctx,
                PriorityQueue::Handle& handle) {
    static std::atomic_size_t global_count{0};
        ...
        [&handle](std::size_t n) {
            ...
            do {
                ...
                current = local_count == 0
                              ? global_count.load(std::memory_order_relaxed)
