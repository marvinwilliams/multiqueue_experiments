...
using Policy = std::variant<scheduling::Normal, scheduling::Idle,
                            scheduling::Fifo, scheduling::RoundRobin>;
  ...
  std::visit(
      [](auto &&p) {
        ...
        if constexpr (policy_type::has_priority) {
          if (p.priority < sched_get_priority_min(p.id) ||
              p.priority > sched_get_priority_max(p.id)) {
...
struct thread_config {
  ...
  thread_config() noexcept { cpu_set.set(); }
...
struct invoker : invoker_base {
  ...
  invoker(Callable func, std::tuple<Args...> arg_tuple)
      : f{std::move(func)}, args{std::move(arg_tuple)} {}
...
class pthread {
  ...
  static void init_attr_with_config(pthread_attr_t *attr,
                                    sched_param *sched_param_ptr,
 ...
 public:
  ...
  pthread(thread_config const &config, Callable &&f, Args &&...args) {
    ...
    auto invoker_ptr = std::unique_ptr<invoker_base>{new invoker_t(
    ...
    int rc = pthread_create(&(*thread_handle_), &attr, trampoline,
                            invoker_ptr.get());
  ...
  pthread(Callable &&f, Args &&...args) {
    ...
    thread_handle_ = pthread_t{};
    int rc =
        pthread_create(&(*thread_handle_), NULL, trampoline, invoker_ptr.get());
    if (rc != 0) {
      throw std::system_error{rc, std::system_category(),
                              "Failed to create thread: "};
    }
