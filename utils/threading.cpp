...
void pthread::init_attr_with_config(pthread_attr_t *attr_ptr,
  ...
  if (config.detached) {
    ...
    if (rc != 0) {
      throw std::system_error{rc, std::system_category(),
                              "Failed to set to detached state: "};
    }
  }
  if (config.policy) {
    ...
    if (rc != 0) {
      throw std::system_error{rc, std::system_category(),
                              "Failed to set explicit scheduling for thread: "};
    }
    std::visit(
        [&](auto &&p) {
...
void pthread::detach() {
  if (!thread_handle_) {
    throw std::runtime_error{
        "Thread object does not represent an active thread of execution"};
  }
  if (detached_) {
